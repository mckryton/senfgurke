...
Public Function execute_example(example As Collection, feature_object As Variant) As Collection
    ...
    For Each step In example(TExampleRunner.EXAMPLE_ATTR_STEPS)
        ...
        step_result = TStepRunner.execute_step(step)
        ...
        TReport.report TReport.TYPE_STEP, step(STEP_ATTR_HEAD) & " " & step(STEP_ATTR_NAME), step_result(0), err_msg
...
error_handler:
    If Err.Number = ERR_ID_SCENARIO_SYNTAX_ERROR Then
        TReport.Log.error_log "syntax error: " & Err.Description & vbCr & vbLf & "in line >" & _
            step(STEP_ATTR_HEAD) & " " & step(STEP_ATTR_NAME) & "<"
    Else
...
Private Function create_example_statistics() As Collection
    ...
    new_example_statistics.Add 0, STEP_RESULT_OK
    new_example_statistics.Add 0, STEP_RESULT_MISSING
    new_example_statistics.Add 0, STEP_RESULT_PENDING
    new_example_statistics.Add 0, STEP_RESULT_FAIL
