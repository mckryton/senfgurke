Attribute VB_Name = "TExampleRunner_Old"
Option Explicit
...
Public Sub run_example(example_steps As Variant, feature_object As Variant)
    ...
    TExampleRunner_Old.IsTestStopped = False
    step_index = 0
    ...
    Do While TExampleRunner_Old.IsTestStopped = False And step_index <= UBound(example_steps)
        ...
        If Not step_result(0) = "OK" Then
            ...
            TExampleRunner_Old.stop_example
        End If
...
error_handler:
    ...
    Else
        TReport.Log.log_function_error "Runtime errror in TExampleRunner_Old.runExample", Join(example_steps, vbTab & vbCr & vbLf)
    End If
End Sub
...
Public Sub stop_example()
    TExampleRunner_Old.IsTestStopped = True
End Sub
